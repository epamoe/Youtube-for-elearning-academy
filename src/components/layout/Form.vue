<template>
  <form
      class="form relative w-fit mx-auto border-1 border-black pl-5 pr-0 rounded-lg"
    >
      <div class="grid grid-cols-12 p-0">
        <div class="col-span-7 border-r-1 border-black m-1">
          <a href="http://">
            <svg
              class="svg absolute top-3 left-2"
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.0057 8.80503H9.37336L9.14923 8.58891C9.93368 7.67639 10.4059 6.49171 10.4059 5.20297C10.4059 2.32933 8.07662 0 5.20297 0C2.32933 0 0 2.32933 0 5.20297C0 8.07662 2.32933 10.4059 5.20297 10.4059C6.49171 10.4059 7.67639 9.93368 8.58891 9.14923L8.80503 9.37336V10.0057L12.8073 14L14 12.8073L10.0057 8.80503ZM5.20297 8.80503C3.20983 8.80503 1.60091 7.19611 1.60091 5.20297C1.60091 3.20983 3.20983 1.60091 5.20297 1.60091C7.19611 1.60091 8.80503 3.20983 8.80503 5.20297C8.80503 7.19611 7.19611 8.80503 5.20297 8.80503Z"
                fill="#777777"
              />
            </svg>
          </a>

          <input
            type="text"
            class="
              focus:ring-0
              form-input
              ml-2
              p-1
              outline-none
              border-none
              focus:outline-0 focus:border-0
              border-l-2
              mx-2
              input
              w-4/5
            "
          />
        </div>

        <div class="col-span-3">
          <select name="" id="" class="border-none focus:ring-0">
            <option value="">Categories</option>
            <option value="">Categories</option>
            <option value="">Categories</option>
            <option value="">Categories</option>
          </select>
        </div>

        <div class="col-span-2">
          <button
            type="submit"
            class="bg-gray-400 rounded-lg p-2 px-5 text-black w-full"
          >
            Seach
          </button>
        </div>
      </div>
    </form>
</template>

<script>
export default {

}
</script>

<style>

</style>